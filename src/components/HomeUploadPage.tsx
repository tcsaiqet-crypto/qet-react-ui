...
export const HomeUploadPage: React.FC<HomeUploadPageProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold qet-badge-accent">
              ...
              <span>QET Automated Agents Orchestration</span>
            </div>
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1.5 text-xs rounded-lg qet-card-elevated">
              ...
              <span style={{ color: 'var(--qet-text-muted)' }}>Pipeline Status:</span>
              <span className="font-bold" style={{ color: 'var(--qet-text-primary)' }}>
                {isUnderstandingReady ? 'AI Understanding Complete' : isUnderstandingRunning ? 'AI Understanding Processing' : hasDocsUploaded && hasZipUploaded ? 'Ready for AI Understanding' : 'Awaiting Document & Codebase Upload'}
              </span>
            </div>
          </div>
      ...
      {/* ── 2. Top Banner: Subagent Stream & Live Status Tracker ──── */}
      <div className="qet-orchestration-container p-5 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b" style={{ borderColor: 'var(--qet-border)' }}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="qet-badge-accent px-2 py-0.5 text-[10px] font-bold font-mono">
                  {completedSubagentsCount} / {subagents.length} Complete
                </span>
              </div>
              <p className="text-xs" style={{ color: 'var(--qet-text-muted)' }}>
                Real-time subagent task execution and progress tracking
              </p>
            </div>
          </div>

          <div className="flex items-center gap-3 self-start sm:self-auto">
            <span className="qet-badge-neutral px-2.5 py-1 text-xs font-mono">
              Generation {appState?.reset_generation || 1}
            </span>
            <span className="qet-badge-accent px-3 py-1 text-xs font-mono font-bold">
              {progress.toFixed(0)}% Run Progress
            </span>
          </div>
        </div>

        {/* 4 Subagents Horizontal Grid */}
            ...
            <div
              ...
              className={`p-3.5 rounded-xl transition-all border ${
                sub.status === 'completed'
                  ? 'qet-badge-success border-emerald-500/30'
                  : sub.status === 'running'
                  ? 'qet-badge-accent ring-2 ring-blue-500/40 shadow-sm border-blue-500/50'
                  : 'qet-card-elevated border-slate-700/30 opacity-75'
              }`}
            >
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="space-y-0.5 min-w-0">
                  <div className="text-xs font-semibold truncate" style={{ color: 'var(--qet-text-primary)' }}>
                    {sub.desc}
                  </div>
                  <div className="text-[11px] leading-tight text-slate-400 truncate">
                    {sub.message}
                  </div>
                </div>
              </div>
      ...
      {/* ── 3. Dual Upload Lanes: Left & Right with Automatic Metrics Card & Tiny Reupload ──── */}
