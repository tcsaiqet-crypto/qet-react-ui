...
Title: ${tc.title}
Category: ${tc.case_type || 'Positive'} | Feature Area: ${tc.feature_area || 'CFA Onboarding Application'}
Generated by: QET Playwright Automation Agent
...
def test_${caseIdClean}(page: Page, target_url: str):
    ...
    Scenario: ${tc.title}
    """
    # 1. Initialize Page Object Model & Navigate
    portal = CFAPortalPage(page, base_url=target_url)
    portal.navigate_to_portal()

    # 2. Ingest Synthetic Test Data & Execute Actions
    ...
    # 3. Capture Pre-assertion Visual Evidence
    page.screenshot(
        path=f"artifacts/screenshots/${tc.case_id}_EXEC.png",
        full_page=True
    )

    # 4. Perform Scenario Verification Assertions
    ${isNegative
...
export const DashboardWorkspace: React.FC<DashboardWorkspaceProps> = ({
  ...
  targetCases.forEach((tc) => {
    if (!scriptMap[tc.case_id]) {
      scriptMap[tc.case_id] = generatePlaywrightScriptForCase(tc);
    }
  });
...
<body>
  <div class="container">
    ...
    <div class="metrics-grid">
      ...
      <div class="card"><div style="font-size:12px;color:#64748B;">FAILED / DEFECTS</div><div class="card-num" style="color:#B91C1C;">${failedCount}</div></div>
    </div>
    ...
    <table>
      ...
      <tbody>
        ${targetCases.map(tc => {
          const isFailed = caseStatusMap[tc.case_id] === 'FAILED';
          return `<tr>
      ...
      <table>
        ...
        <tbody>
          ${targetCases.map((tc, i) => {
            const failed = caseStatusMap[tc.case_id] === 'FAILED';
  ...
  return (
    <div className="space-y-6 animate-fade-in p-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="qet-card p-4 flex items-center justify-between bg-white border-l-4 border-emerald-500">
          <div><p className="text-[10px] font-bold text-slate-500 uppercase">Pass Rate</p><p className="text-2xl font-bold text-emerald-600">{passRate}%</p></div>
        </div>
        <div className="qet-card p-4 flex items-center justify-between bg-white border-l-4 border-slate-500">
          <div><p className="text-[10px] font-bold text-slate-500 uppercase">Total Cases</p><p className="text-2xl font-bold text-slate-800">{targetCases.length}</p></div>
        </div>
        <div className="qet-card p-4 flex items-center justify-between bg-white border-l-4 border-emerald-500">
          <div><p className="text-[10px] font-bold text-slate-500 uppercase">Passed</p><p className="text-2xl font-bold text-emerald-600">{targetCases.filter(tc => !caseStatusMap[tc.case_id] || caseStatusMap[tc.case_id] === 'PASSED').length}</p></div>
        </div>
        <div className="qet-card p-4 flex items-center justify-between bg-white border-l-4 border-rose-500">
          <div><p className="text-[10px] font-bold text-slate-500 uppercase">Failed / Bugs</p><p className="text-2xl font-bold text-rose-600">{targetCases.filter(tc => caseStatusMap[tc.case_id] === 'FAILED').length}</p></div>
        </div>
      </div>
      ...
      {/* ── Defects & Bug Tracking Card (2 or 3 failed cases with bugs) ── */}
      {metrics.defects && metrics.defects.length > 0 && (
        <div className="qet-card p-5 space-y-4 border border-rose-200 bg-rose-50/40">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ShieldAlert className="w-5 h-5 text-rose-700" />
              <h3 className="text-xs font-bold uppercase tracking-wider text-rose-900">
                Defects & Bug Tracking ({metrics.defects.length} Issues Identified)
              </h3>
            </div>
            <span className="text-[10px] font-bold text-rose-800 bg-rose-100 px-2 py-0.5 rounded border border-rose-200">
              {metrics.defects.length} Failed Scenarios Triaged
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            {metrics.defects.map((bug) => (
              <div key={bug.bug_id} className="p-3.5 bg-white rounded-xl border border-rose-200 space-y-2 shadow-2xs">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <span className="font-mono font-bold text-rose-900">{bug.bug_id}</span>
                    <span className="font-mono text-[10px] text-slate-500">({bug.test_case_id})</span>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                    bug.severity === 'Critical' || bug.severity === 'High' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'
                  }`}>
                    {bug.severity}
                  </span>
                </div>
                <p className="font-semibold text-slate-800 line-clamp-1">{bug.title}</p>
                <p className="text-[11px] text-slate-600 line-clamp-2 leading-relaxed">{bug.description}</p>
                <div className="p-2 bg-slate-50 rounded-lg text-[10px] text-slate-700 font-mono border border-slate-200">
                  <span className="font-bold text-rose-700">Root Cause:</span> {bug.root_cause}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="qet-card p-4 flex flex-wrap items-center justify-between gap-3 bg-white">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors flex items-center gap-1.5 border border-indigo-700 shadow-xs cursor-pointer"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-colors flex items-center gap-1.5 shadow-xs cursor-pointer"
          >
            ...
            <div key={tc.case_id} className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-2.5 py-1 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-lg flex items-center gap-1 transition-colors border border-slate-200 shadow-2xs cursor-pointer"
                >
                ...
                <button
                  onClick={() => setActiveModal({ type: 'script', caseId: tc.case_id, content: script?.code || generatePlaywrightScriptForCase(tc).code })}
                  className="px-2.5 py-1 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-lg flex items-center gap-1 transition-colors border border-slate-200 shadow-2xs cursor-pointer"
                >
                  ...
                  <div className="rounded-xl overflow-hidden border border-slate-300 shadow-md bg-white">
                    ...
                    <div className="p-6 bg-slate-50 min-h-[300px] flex flex-col justify-between space-y-4">
                      ...
                      <div className="flex items-center justify-between text-[11px] text-slate-400 pt-2 border-t border-slate-200">
                        ...
                        <button
                          onClick={handleDownloadHtmlReport}
                          className="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1 cursor-pointer"
                        >
              ...
              {activeModal.type === 'allure' && (
                <div className="w-full h-full min-h-[400px] flex flex-col items-center justify-center text-center space-y-3 font-sans">
                  <div className="space-y-2">
                    ...
                    <button
                      onClick={handleDownloadHtmlReport}
                      className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-bold shadow-xs transition-colors cursor-pointer"
                    >
